...
public class GameManager : CITEPlayer
{
    ...
    private void FixedUpdate()
    {
        foreach (var touch in Input.touches)
        {
            ...
            foreach (var hit in colliders)
            {
                ...
                if (obj.CompareTag("TempleButton")) {
                    PressButton(touch, obj);
                }
    ...
    private void PressButton (Touch touch, GameObject button)
    {
        switch (touch.phase)
        {
            case TouchPhase.Began:
                button.GetComponent<TempleButtonScript>().OnInputDown();
                if (!_cubesTouched.ContainsKey(touch.fingerId))
                    _cubesTouched.Add(touch.fingerId, button);
                break;
            case TouchPhase.Ended:
            case TouchPhase.Canceled:
                button.GetComponent<TempleButtonScript>().OnInputUp();
                _cubesTouched.Remove(touch.fingerId);
                break;
            case TouchPhase.Stationary:
                button.GetComponent<TempleButtonScript>().OnInputDrag();
                break;
            default:
