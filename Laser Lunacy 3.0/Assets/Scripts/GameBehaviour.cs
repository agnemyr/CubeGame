...
public class GameBehaviour : CITEPlayer
{
    ...
    */
    ...
    {
        var canvas = GameObject.Find("CountdownCanvas");
        if (canvas == null) return;
        var timer = canvas.GetComponentInChildren<CountDownTimer>();
    ...
    [Command]
    public void CmdUpdateZones(GameObject cube, GameObject targetZone, GameObject zone)
    {

        if (cube.CompareTag("Cube") && targetZone.GetComponent<ZoneBehaviour>().listIsFull() == false)
        {
            Vector3 newPos = targetZone.GetComponent<ZoneBehaviour>().GetSpawnPosition();

           CmdUpdateCubePosition(cube, newPos);
    ...
    [ClientRpc]
    public void RpcUpdateZones (GameObject cube, GameObject targetZone, GameObject zone)
    {
        if (cube.CompareTag("Cube") && targetZone.GetComponent<ZoneBehaviour>().listIsFull() == false)
        {
            Vector3 newPos = targetZone.GetComponent<ZoneBehaviour>().GetSpawnPosition();
    ...
    [Command]
    public void CmdDoTeleport (GameObject teleportPad, GameObject cube)
    {
        teleportPad.GetComponent<TeleportCube>().DoTeleport(cube);
    }
